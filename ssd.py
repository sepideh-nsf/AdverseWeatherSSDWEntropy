...
def Entropy(Input):
    ...
    #print(torch.min(Input).item())
    #print(torch.max(Input).item())
    ...
    Patches=torch.nn.functional.unfold(Input, (16,16), padding=15)
    Patches=Patches.view(N, C, 16*16, 315,315)[:,:,:,15:,15:].reshape(N, C, 16*16, -1).swapaxes(-2, -1)
    #Patches = Patches.view(N, C, 16*16, -1).swapaxes(-2, -1)
    ...
    print(Patches.shape)
    #Patches = Patches.view(-1, 16*16)#.contiguous()
    # minim=torch.min(Patches[Patches!=-1])
    # maxim=int(torch.max(Patches))
    # p=Patches.nonzero()
    #del PaddedInput
    #torch.cuda.empty_cache()
    #minim = int(Patches.min().item())
    #maxim = int(Patches.max().item())
    #numclasses=maxim-minim
    start=time.time()
    del Input
    torch.cuda.empty_cache()
    # Hists=torch.histc(Patches[0],bins=maxim-minim,min=minim,max=maxim)
    # https://github.com/pytorch/pytorch/issues/99719
    Hists = torch.zeros((*Patches.shape[:-1], 255), dtype=Patches.dtype, device=Patches.device)
    ones = torch.tensor(1, dtype=Patches.dtype, device=Patches.device).expand(Patches.shape)
    Hists.scatter_add_(-1, Patches.long(), ones)
    print(Hists.shape)
    #Hists=[torch.histc(item,bins=maxim-minim,min=minim,max=maxim) for item in Patches]
    ...
    #Ent = (Hists * torch.where(Hists < 1e-5, 0, torch.log(Hists))).sum(dim=-1)
    ...
    Ent=torch.nan_to_num(Hists, nan=0).sum(dim=-1)
    #del Hists
    #torch.cuda.empty_cache()
    entropy=Normalize(Ent.view(size),-1)
    #print(entropy.min().item())
    #print(entropy.max().item())
    return entropy
    # for m in range(w):
    #     for n in range(h):
...
class SSD(nn.Module):
    ...
    def forward(self,x):
        ...
        try:
            ...
            InjectIndxs=[3,8,15,22,29,32,34]
            ChannelSizes=[64,128,256,512,512,1024,1024]
            ######################
            # t = time.time()
            # try:
            #     entropy = Uncertainty.cpu().detach().numpy()
            #     for it in range(entropy.shape[0]):
            ...
            print("size of Uncertainty:",Uncertainty.shape)
            # apply vgg up to fc7
            for k in range(23, len(self.vgg)):
                ...
                if k in InjectIndxs:
                    if k==32:
                        ConvLayer=nn.Conv2d(3, 1024, kernel_size=3, padding=6, dilation=6)
                    elif k==34:
                        ConvLayer=nn.Conv2d(3, 1024, kernel_size=1)
                    else:
                    ...
                    x=torch.cat((MultEntr, Uncertainty), 1)
                    if k==29:
                        MaxpoolLayer=nn.MaxPool2d(kernel_size=3,stride=1, padding=1)
                        Uncertainty=MaxpoolLayer(Uncertainty)
            sources.append(x)
            
            ChannelSizesExtras=[512,256,256,256]
            print("size of x:",x.shape)
            print("size of Uncertainty:",Uncertainty.shape)
            ...
            for k, v in enumerate(self.extras):
                ...
                if k % 2 == 1:
                    if ite<2:
                        ConvLayer=nn.Conv2d(3, ChannelSizesExtras[ite], kernel_size=3, stride=2,padding=1)
                        print("here")
                    else:
                        ConvLayer=nn.Conv2d(3, ChannelSizesExtras[ite], kernel_size=3)
                        print("There")
                    ConvUncert=ConvLayer(Uncertainty)
                    sig = nn.Sigmoid()
                    SigUncert = sig(ConvUncert)
                    ...
                    print("size of x:",x.shape)
                    print("size of Uncertainty:",Uncertainty.shape)
                    Shape=torch.cat((torch.tensor(Uncertainty.shape[:2]),torch.tensor(x.shape[-2:]))).tolist()
                    print(Shape)
                    Uncertainty=F.interpolate(Uncertainty,x.shape[-2:] , mode='nearest')
                    print("size of Uncertainty:",Uncertainty.shape)
                    # if ite < 2:
                    #     x = torch.cat((MultEntr, SigUncert), 1)
                    #     print("here")
                    # else:
                    x=torch.cat((MultEntr, Uncertainty), 1)
                    sources.append(x)
                    ite+=1
                # else:
    #                 ConvLayer=nn.Conv2d(in_chan, int(ChannelSizesExtras[ite]/2), kernel_size=1)
    #                 ConvUncert=ConvLayer(Uncertainty)
    #                 sig = nn.Sigmoid()
    #                 Uncertainty = sig(ConvUncert)
    #                 in_chan=int(ChannelSizesExtras[ite]/2)
    #                 print("size of x:",x.shape)
    #                 print("size of Uncertainty:",Uncertainty.shape)

            # apply multibox head to source layers
            iterat=range(len(sources))
            for (itemC,x, l, c) in zip(iterat,sources, self.loc, self.conf):
                print(itemC)
...
def vgg(cfg, i, batch_norm=False):
    ...
    conv6 = nn.Conv2d(515, 1024, kernel_size=3, padding=6, dilation=6)
    conv7 = nn.Conv2d(1027, 1024, kernel_size=1)
    layers += [pool5, conv6,
...
#                           cfg[k] * num_classes, kernel_size=3, padding=1)]
    for k, v in enumerate(extra_layers[1::2], 2):
        loc_layers += [nn.Conv2d(v.out_channels+3, cfg[k]
                                 * 4, kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(v.out_channels+3, cfg[k]
