...
parser.add_argument('--batch_size', default=8, type=int,#32 Works fine
...
parser.add_argument('--lr', '--learning-rate', default=5e-4, type=float,#1e-3 works fine
...
def train():
    ...
    if args.resume:
        ...
        # vgg_weights = torch.load(args.save_folder +"ssd300_COCO_115000.pth")# args.basenet)
        print('Loading base network...')
        # state=ssd_net.state_dict()
        # state=ssd_net.vgg.state_dict()
        # for k,v in zip(state.keys(), weights.values()):#vgg_weights.values()):
        #     # try:
        ...
        ssd_net.vgg.load_state_dict(vgg_weights)##################with this worked fine
    ...
    if not args.resume:
        ...
        #ssd_net.vgg.apply(weights_init)##############with this commented worked fine
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        ...
        if iteration != 0 and iteration % 20 == 0:
            ...
            torch.save(ssd_net.state_dict(), 'weights/ssd300VGGEntropy_VOC_' +
                       repr(iteration) + '.pth')
